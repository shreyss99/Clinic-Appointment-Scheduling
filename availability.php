<?php
	...
	$sql = "SELECT timeslot FROM Appointment where Type='$roomtype' AND Choice='$roomchoice' AND View='$roomview'";
	...
	if ($result->num_rows > 0)
	{
		...
		for($i=0;$i<$result->num_rows;$i++)
		{
			...
			if($result2->num_rows == 0)  //that room is not booked
			{
				...
				$query2 = "INSERT INTO reservation values(default,'$pat_id',".(int)$x.",'$checkin','$checkout')";
				echo $query2;
		...
		if($roombooked==false)
		{
			for($j=0;$j<$result->num_rows;$j++) 
			{
				...
				if($val==$result2->num_rows)
				{
					...
					$_SESSION['checkout']=$checkout;
					header("Location:confirmbooking.php");	
					/*$query2 = "INSERT INTO Appointment values(default,'$pat_id',".(int)$x.",'$checkin','$checkout')";
					$mysqli->query($query2) or die("insert error");
					$roombooked=true;
